...
def totp_code(
    ...
    EvidPrism) during authorised VAPT, where the client provides a
...
class AuthSession:
    ...
    def auto_login(
        ...
        Supports MFA-protected apps such as EvidPrism. When the supplied
        ``login_path`` is the EvidPrism contract (``auth/login``), the
        EvidPrism JSON shape is tried first:
        ...
        login_surface
            EvidPrism workspace selector. Defaults to ``workspace``;
            callers must explicitly request ``superadmin`` to test the
            admin surface.
        admin_path
            EvidPrism admin path, only sent when ``login_surface`` is
            ``superadmin``.
        ...
        # EvidPrism path — explicit JSON contract for /auth/login.
        is_evidprism_path = login_path.strip("/").lower() == "auth/login"
        payload_attempts: list[dict] = []
        if is_evidprism_path:
            evid_body: dict = {
                "email": username,
                "password": password,
        ...
        for payload_kwargs in payload_attempts:
            ...
            # EvidPrism / generic: response body token (JWT or opaque).
            token = ""
