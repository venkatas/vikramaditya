...
python3 dorks.py -d clienta.com            # all categories → recon/<target>/sessions/<id>/passive/
python3 dorks.py -d clienta.com -c microsoft365   # M365 tenant only
python3 vikramaditya.py clienta.com --passive-only  # scoping-call mode
python3 vikramaditya.py clienta.com --skip-passive  # legacy behaviour
```
...
`python3 -m py_compile dorks.py vikramaditya.py` clean. `python3 dorks.py --list` returns 13 categories with counts. `python3 dorks.py -d clienta.com -c credentials --output-dir /tmp/smoke` produces 14 queries with valid HTML+JSON+TXT artifacts.
...
Same target (`clientc-spa.clientb.com`), same args, parallel runs:
...
Benchmark on M-series (3 models × 5 triage + 3 reasoning + speed, num_predict=1024) ran against fresh client-a findings.json from 03 May whitebox run:
...
Four P2 fixes from the clienta engagement.
...
## v9.1.0 — Engagement-driven hardening: P0+P1 fixes from clienta VAPT (2026-05-02)
...
- **Engagement evidence (clienta fresh run):** 11 RDS snapshots flagged "public" by Prowler — all 11 actually private. 7 buckets flagged write-public — all 5 tested are CloudFront-OAC (legit). Without this filter, the v1.7 report would have shipped 18 confirmed false-positives as "CRITICAL".
...
- **Engagement evidence:** Both clienta WAFs were in monitor-only mode for rate-limit, bot-control, and (in client-a) body-SQLi — every test in this engagement was *logged but not blocked*, and Prowler didn't flag it. Eleven rules in client-a's `AWS-EC2-WAF` and seven in client-b's `Exc-Prod-Web-Acl` are now visible to the scanner.
...
- **Engagement evidence:** clienta api-maya endpoint differential (`{"foo":"bar"}` 5xx vs `{"$gt":""}` 5xx — same response = type confusion, not real NoSQLi) drove the verdict logic. Without this differential test, the v1.7 report would have shipped MEDIUM/HIGH findings for endpoints that are actually input-validation flaws.
...
- Live engagement evidence: `maya.clienta.com` was discovered by amass on 1 May (1 hit in `subdomains/amass.txt`) but never appeared in `subdomains/all.txt` (0 hits). When the user asked whether `maya.clienta.com` was in our live list today, it was missing — even though amass had found it the previous day.
...
Re-running the post-processor against the existing `recon/clienta.com/sessions/20260501_120323_e6f0/subdomains/amass.txt` recovered **50 subdomains** that had been silently dropped. Of those, **3 are confirmed live HTTPS** with real applications behind them:

- `maya.clienta.com` → Apache 2.4.58 Ubuntu, 307 → Sign In page
- `dboxconfig.clienta.com` → Microsoft IIS 10.0, 301 "Document Moved"
- `clientc-spa-fm.clienta.com` → Microsoft IIS 10.0, 302 "Object moved"
...
- **Phase 3.5: TLS Cert SAN Harvest** (P11) — runs `tlsx -san -cn` against the live IP list, dedupes, and emits `certs/sans.txt`. Diffed against the existing subdomain enum into `certs/scope_candidates.txt` so any cert-leaked domain that subdomain enum missed gets surfaced. Skips with friendly hint if `tlsx` isn't installed. (Engagement value: surfaced `clientd.com` in 60 sec; subfinder + amass + assetfinder produced 0 hits for it.)
- **Phase 4.5: Visual Recon** (P15) — runs `gowitness scan file -f live/urls.txt` to capture per-URL JPEG screenshots + the rendered DOM title into `screenshots/`. JSONL output (`screenshots/gowitness.jsonl`) is reporter-consumable. Skips with friendly hint if `gowitness` isn't installed. (Engagement value: caught the v1.3 misclassification of `203.0.113.11/signin` as "Apache default install page" — it's actually the production CLIENTC-ERP HRMS payroll Sign-In page.)
...
- **Route 53 → blackbox scope auto-suggest** (P23) — new module `whitebox/correlator/scope_suggest.py`. The orchestrator's correlation phase now writes `<session>/cloud/<account_id>/scope-suggestion.json` listing every client-owned domain reachable from the audited account: Route 53 hosted zones (public only — internal zones excluded), CloudFront aliases (CNAMEs), Internet-facing ELB DNS names, and ELB-name product fragments that often map to product domains. 8 new unit tests including an engagement-reproduction case (the `Exc-Prod-UI-clientd` LB-name → product domain pattern that surfaced clientd.com in this engagement).
...
- `recon.sh` Phase 9 (Exposed Config Files & Sensitive Paths) was emitting
  ...
  1. **SPA catchall** (e.g. `clientc-spa.clientb.com`) — every path returns the
     same `<title>clientc-spa.ai</title>` HTML shell because the host runs a
     React/Vue SPA and the server hands the bundle for unmatched paths.
  2. **Empty-body 200** (e.g. `mtrack.clientd.com`) — IIS / ASP.NET
     routing returns `200 OK` with `Content-Length: 0` for paths the
     application doesn't recognize.
  Total false positives across the 4-day engagement: **87** (clientc-spa 36 +
  mtrack/v2 51).
...
- `clientc-spa.clientb.com`: 36 → **0** false positives
- `mtrack.clientd.com`: 51 → **0** false positives
- `www.clientb.com`: 0 → 0 (no regression)
...
- Discovered during a live full run on `clienta.com` (38 min recon,
