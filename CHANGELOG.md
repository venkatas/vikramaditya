...
## v9.18.0 — TOTP-aware MFA login (EvidPrism + generic) (2026-05-09)

Adds RFC-6238 TOTP support so Vikramaditya can authenticate to MFA-protected
APIs **without disabling MFA** on the target. Built for EvidPrism's
...
EvidPrism (and increasingly every B2B SaaS we engage) enforces TOTP at
...
- **`AuthSession.auto_login()`** now takes
  ...
  the login path is `auth/login` the EvidPrism JSON shape is tried
...
`tests/test_totp_evidprism.py` (17 tests, all passing):
...
- EvidPrism workspace JSON shape includes `loginSurface=workspace`,
...
- `--auth-creds` + `--totp-secret` submits the EvidPrism shape with a
  6-digit code.
...
- `asp.net, iis, dotnetfx=4.8, windows-server` (the 3.111.177.137:8443 eSSL target) → all 🟢 Supported (asp.net→`dotnetfx`, iis→`windows-server` via host OS).
...
python3 brain_model_bench.py \
    --watch-log logs/vikram_pranapr_fresh_20260506_082437.log \
    --watch-pattern "==== END" \
    --findings-glob "findings/pranapr.com/sessions/2026050*" \
    --recon-glob    "recon/pranapr.com/sessions/2026050*"
```
...
`python3 -m py_compile` clean. `vikramaditya.__version__` returns `9.16.0`. Auto-fire watcher launched against current pranapr-fresh run; will produce the first leaderboard once that run completes.
...
python3 vikramaditya.py adfactorspr.com
