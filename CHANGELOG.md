...
Reading the v7.4.8 scan output, the user spotted that `clientc-spa.clientb.com` was returning 200 with a "404 page" body, and Check 0 was happily logging four `[VULN] Found upload path` lines for paths that didn't exist. Tracing through `scanner.sh:170-189` exposed three layered bugs:
...
Verified end-to-end: catchall correctly logged for `clientc-spa.clientb.com`, **zero** `Found upload path` lines (vs. 4 false positives in v7.4.8), no `declare:` / `unbound variable` noise, all 13 checks complete clean.

## v7.4.8 — scanner.sh hardening: 3 bugs caught on a real-world full-scope run (2026-04-27)

Live test against clientb.com (9 subdomains, 14 live HTTP hosts, 5 IPs) surfaced three scanner.sh bugs. Each was reproduced, patched, and verified across two re-runs.
...
### Found by
Single end-to-end full-scope scan against clientb.com — initial run hit bug #1 (mid-Check 3); first verify run hit bug #2 (summary footer); second verify run hit bug #3 (summary footer). Third run printed clean across all 13 checks. Real-target findings: 14 WordPress hosts (xmlrpc + plugin-CVE surface), 8 upload-candidate paths on `clientc-spa.clientb.com`, 12 unauth API endpoints on `radar-testing.clientb.com` (incl. a 1 MB CSV data dump).
