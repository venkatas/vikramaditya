...
Same target (`projx.clientb.com`), same args, parallel runs:
...
- `projx-fm.clienta.com` → Microsoft IIS 10.0, 302 "Object moved"
...
- `recon.sh` Phase 9 (Exposed Config Files & Sensitive Paths) was emitting
  ...
  1. **SPA catchall** (e.g. `projx.clientb.com`) — every path returns the
     same `<title>projx.ai</title>` HTML shell because the host runs a
  ...
  Total false positives across the 4-day engagement: **87** (projx 36 +
  mtrack/v2 51).
...
- `projx.clientb.com`: 36 → **0** false positives
