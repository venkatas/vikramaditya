...
python3 -m whitebox.cloud_hunt --profile client-a \
  --allowlist clienta.com \
  --session-dir recon/clienta.com

# Both accounts in one run
python3 -m whitebox.cloud_hunt --profile client-a --profile client-b \
  --allowlist clienta.com --allowlist clientb.com \
  --session-dir recon/<target>

# Bust the 24h phase cache and re-run everything
python3 -m whitebox.cloud_hunt --profile client-a --refresh \
  --allowlist clienta.com --session-dir <dir>

# Disable scope-lock (audit ALL public Route53 zones in the account)
python3 -m whitebox.cloud_hunt --profile client-a --no-scope-lock \
  --session-dir <dir>
```
...
`tests/whitebox/smoke/test_real_aws.py` against `client-a` and
`client-b` profiles.
