"""Acceptance tests for TOTP + clientk login support.

Covers:
- auth_utils.totp_code() against RFC 6238 vectors and edge cases.
- AuthSession.auto_login clientk JSON shape (workspace + superadmin).
...
- autopilot --auth-creds + --totp-secret submits the clientk shape.
...
# ─── AuthSession.auto_login — clientk contract ──────────────────────────────
class TestclientkAutoLogin:
    ...
    def test_workspace_login_with_totp_secret_returns_token(self, monkeypatch):
        ...
        sess = AuthSession("https://app.clientk.com/api")
    ...
    def test_generic_login_paths_still_work_without_totp(self, monkeypatch):
        # Non-clientk path: server replies on the second attempt (form-data).
        scripted = [
            _FakeResponse(401, {}),
        ...
        # First call should NOT include adminPath (non-clientk) and no TOTP.
...
class TestAutopilotCli:
    ...
    def test_auth_token_skips_login(self, monkeypatch, capsys):
        ...
        run_autopilot(
            base_url="https://app.clientk.com/api",
    ...
    def test_auth_creds_with_totp_secret_submits_clientk_shape(self, monkeypatch, capsys):
        secret = base64.b32encode(b"12345678901234567890").decode()
        scripted = [
        ...
        # Both logins should hit auth/login with clientk JSON shape.
