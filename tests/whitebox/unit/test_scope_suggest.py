...
def test_summary_string(tmp_path):
    inv = _make_inventory(tmp_path,
                          ...
                          elb_lbs=[{"Scheme": "internet-facing", "DNSName": "x.elb.amazonaws.com",
                                    "LoadBalancerName": "Exc-Prod-clientd"}])
    s = build_scope_suggestion(inv)
...
def test_engagement_scenario_finds_clientd(tmp_path):
    ...
    inv = _make_inventory(tmp_path,
                          route53_zones=["clienta.com", "example-clientc.com"],
                          elb_lbs=[
                              {"Scheme": "internet-facing",
                               "DNSName": "Exc-Prod-UI-clientd-0000000000.ap-south-1.elb.amazonaws.com",
                               "LoadBalancerName": "Exc-Prod-UI-clientd"},
                          ])
    s = build_scope_suggestion(inv)
    # Operator can spot "clientd" in the LB name list
    assert any("clientd" in n for n in s["elb_product_names"])
