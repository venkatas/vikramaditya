...
def test_confirmed_injection_drives_exploit_loop(tmp_path, monkeypatch):
    ...
    req.write_text("POST https://ttdtcp.example.invalid/TT/X HTTP/1.1\n"
                   "Host: ttdtcp.example.invalid\n\n{\"prefixText\":\"s\"}\n")
    fake = _wire(monkeypatch, tmp_path, (True, REAL_CONFIRMED))

    result = hunt.run_sqlmap_request_file(str(req), domain="ttdtcp.example.invalid")
    ...
    assert call["target_url"].startswith("https://ttdtcp.example.invalid/")
