...
**v7.4.5 — empty-findings diagnostic section: reporter now explains *what was scanned* (recon counts + target-shape hints) so a 0-findings report is never blank + reporter subdir coverage: 5 silently-dropped finding classes restored (mfa, saml, deserialize, import_export, supply_chain) + unknown-subdir warning log + README honesty pass + NAT64 FP fix + severity-spelling fix + email_audit per-check package + brain.py LLM bridge + hunt_journal auto-append + 552-test suite + email-auditor agent + sqlmap JSON API detection + anonymization proxy + sneaky_bits + /autopilot + /remember + /surface + recon-ranker + meme-coin + /intel + bb-methodology + credential store + /pickup + CI/CD + HackerOne MCP + CVSS 4.0 + HAR auth testing**
