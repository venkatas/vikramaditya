...
```
$ python3 vikramaditya.py is.clientg.com

  Scope lock? (scan this exact host only, no subdomain expansion) [y/N]: y
  [scans is.clientg.com only — no www.clientg.com, no ishare.clientg.com]
```
...
The `--verify-fix` mode is what caught the CGI-webmail OR-logic flaw: developers claimed they fixed the file upload, but the brain read the actual PHP code, spotted `if (mime_ok OR ext_ok)` (should be AND), and wrote the bypass PoC that regained RCE.
