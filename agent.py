...
class ToolDispatcher:
    ...
    def _url_in_scope(self, url: str) -> bool:
        ...
        ``--scope-lock`` promises exact-host isolation. Subdomains are therefore
        out of scope unless the operator starts a separate exact-host task for
        that subdomain.
        """
    ...
    *,
    ...
    if not recon_dir:
        return {
            "success": False,
            "domain": domain,
            "error": f"Recon session not found: {resume_session_id}",
        }
