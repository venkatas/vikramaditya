...
- **Targets:** `clienta.com` (AWS profile `client-a`, account `000000000001`), `clientb.com` (AWS profile `client-b`, account `000000000002`).
...
- **Real-account smoke test:** `tests/smoke/test_real_aws.py` (gated by env var, runs only with explicit opt-in) hits both `client-a` and `client-b` for sanity check during development.
...
14. End-to-end test on `client-a` + `client-b`
