...
**Goal:** Add a self-contained `whitebox/` package that audits AWS accounts (`client-a` → 000000000001, `client-b` → 000000000002), pulls IAM/secrets/exposure data, correlates with blackbox findings, and produces an integrated Burp-style HTML report.
...
def test_validate_calls_sts(tmp_path):
    ...
    fake_sts.get_caller_identity.return_value = {
        "UserId": "AID", "Account": "000000000001",
        "Arn": "arn:aws:iam::000000000001:user/venkata.satish-audit",
    }
    ...
    with patch("boto3.Session", return_value=fake_session):
        prof = validate(CloudProfile(name="client-a"))

    assert prof.account_id == "000000000001"
...
def test_enumerate_returns_zone_names(profile):
    ...
    r.create_hosted_zone(Name="clienta.com.", CallerReference="x")
    r.create_hosted_zone(Name="clientb.com.", CallerReference="y")
    profile._session = boto3.Session(region_name="us-east-1")

    zones = enumerate_zones(profile)
    names = sorted(z["name"] for z in zones)
    assert names == ["clienta.com", "clientb.com"]
...
def test_cli_calls_orchestrator_with_profile(monkeypatch, tmp_path):
    ...
    assert kwargs.get("profile_name") == "client-a" or "client-a" in args
```
...
@pytest.mark.parametrize("profile_name,expected_account", [
    ("client-a",     "000000000001"),
    ("client-b", "000000000002"),
])
...
@pytest.mark.parametrize("profile_name", ["client-a", "client-b"])
...
git commit -m "test(whitebox): opt-in smoke test for client-a + client-b profiles"
```
...
python3 -m whitebox.cloud_hunt --profile client-a --session-dir recon/clienta.com/sessions/<id>

# Both accounts
python3 -m whitebox.cloud_hunt --profile client-a --profile client-b \
  --session-dir recon/<target>/sessions/<id>

# Bust the 24h phase cache and re-run everything
python3 -m whitebox.cloud_hunt --profile client-a --refresh --session-dir <dir>
```
...
Run:
```
python3 -m whitebox.cloud_hunt --profile client-a \
  --session-dir recon/clienta.com/sessions/whitebox-smoke
```
Expected: completes (may take 30–60 min). Inspect `recon/clienta.com/sessions/whitebox-smoke/cloud/000000000001/findings.json`.

- [ ] **Step 5: Generate full report**

Run:
```
python3 reporter.py recon/clienta.com/sessions/whitebox-smoke/findings/ \
  --client "ADF" --target clienta.com
```
Inspect the HTML output for the "Cloud Posture — Account 000000000001" chapter and any inline cloud context.
...
git add recon/clienta.com/sessions/whitebox-smoke/reports/
git commit -m "test(whitebox): smoke run report artifacts for client-a"
```
