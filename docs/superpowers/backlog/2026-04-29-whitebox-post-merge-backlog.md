...
Filed 2026-04-29 after live validation against `client-b` (account 000000000002)
and cross-check with the Example-ClientcPR AWS Posture report dated 29-Apr-2026.
...
**Evidence from 29-Apr live run (client-b).** Five SGs flagged for "MSSQL 1433
from 0/0":

- `EXAMPLE-CLIENTC-PROD-RDS-SG`, `example-clientc-rds-3-ondemand`, `example-clientc-rds-3-uat`,
  `radar-boundary-box-lambda-SG` — **0 instances attached** (orphan rules).
- `EXC-OpenVPN` (`13.232.207.52`) — 1 instance with public IP, but `:1433` and
  `:5432` time out on direct TCP probe (filtered upstream).
- `EXAMPLE-CLIENTC-PROD-ES-ASG` "9200 + 5601 from 0/0" — host `EXC-ElasticSearch`
...
**Evidence.** Manual report: "clientb `security-alerts` SNS topic — zero
...
absent — EC2AMZ-D8IH8H1 not SSM-managed; ~37% of clientb running fleet
...
**Problem.** The 29-Apr live run against client-b (account 000000000002)
...
client-b; secrets dir fills with 145 files but manifest.json never gains a
`secrets` key.
...
**Evidence (29-Apr live run on client-a, account 000000000001).** Run hung at
...
**Acceptance.** Live run against client-a completes the secrets phase in
under 5 minutes regardless of opt-in region status.

---

## P9 — client-a live validation results (29-Apr)
...
| **prowler** | **complete** | **3,750 OCSF findings**: severity 5 (HIGH) 1,317; sev 4 (MEDIUM-HIGH) 681; sev 3 (MEDIUM) 1,517; sev 2 (LOW) 234; sev 1 (INFO) 1. Plus CIS 1.5 / SOC2 / HIPAA / GxP compliance CSVs. **First successful Prowler completion** of the engagement (clientb timed out). |
