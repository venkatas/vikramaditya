...
domain `merryspiders.com` and the subdomain `projx-dashboards` during the
...
- projx.ai UAT (3.109.133.130) — undisclosed staging environment
...
- The projx.clientb.com SPA-catchall would have shipped 36 false-positive
...
**Engagement evidence.** projx.clientb.com — 36 false-positive paths
...
**Acceptance.** A re-run against projx.clientb.com produces 0 entries in
...
account also matching `clienta*` / `clientb*` / `merryspiders*` prefixes.
...
Route 53 (ad-factors.com, clienta-{dev,prod,uat}.com, plus indirectly
...
false positive (projx SPA-catchall) was re-discovered three times. The
