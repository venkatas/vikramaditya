...
**Engagement evidence.** The CLIENTC-ERP HRMS app at `203.0.113.11/signin` is a
Next.js application. Its JS chunks reveal **two in-scope subdomains** that no
passive subdomain source surfaced:

- `hrms-organisation-gateway.clienta.com`
- `hrms-user-gateway.clienta.com`
...
domain `clientd.com` and the subdomain `clientc-spa-dashboards` during the
...
clienta.com
cloudfront.net
crawler-testing.clientb.com
clientc-spa-dashboards         ← new
clientd.com         ← new — third client domain
clientb.com
radar-edition-testing.clientb.com
```
...
- clientc-spa.ai UAT (203.0.113.12) — undisclosed staging environment
- GLPI ticketing system (203.0.113.13) — known-CVE software
- 4× IIS 10.0 default-install pages on production hosts
- Apache default install page on `Example-Clientc-Erp-Web-2`
...
- `clientb.com`: DMARC missing entirely → freely spoofable
- `clientb.com`: DKIM not visible at any common selector
...
- The 4 of 5 "MSSQL 1433 from 0/0" SGs in client-b (orphan rules with
  no instances) would have shipped as "5 critical exposures" without
  reachability gating. Real exploitable count was 0.
- The clientc-spa.clientb.com SPA-catchall would have shipped 36 false-positive
...
`203.0.113.11:443` was reported as "Apache default install page" in v1.3
of the AWS-team report. Screenshots showed it's actually the **CLIENTC-ERP HRMS
...
**Engagement evidence.** clientc-spa.clientb.com — 36 false-positive paths
...
**Acceptance.** A re-run against clientc-spa.clientb.com produces 0 entries in
...
**Acceptance.** WP cluster (28 hosts on clientb.com today) gets a clean
...
buckets in `client-b` (heuristic-scanned 10 of them for secrets). External
`cloud_enum` would tell us whether there are buckets *outside* that
account also matching `clienta*` / `clientb*` / `clientd*` prefixes.
...
**Problem.** During the engagement, 5 client domains existed in client-b
Route 53 (example-clientc.com, clienta-{dev,prod,uat}.com, plus indirectly
clientd.com via the `Exc-Prod-UI-clientd` ELB name) but the
original blackbox scope was just 2 domains (`clienta.com`,
`clientb.com`). The other 5 were discovered manually mid-engagement.
...
false positive (clientc-spa SPA-catchall) was re-discovered three times. The
...
| P15 | Visual recon (gowitness) | S | Medium — corrected the CLIENTC-ERP finding |
